...
- Cargo workspace root is `crates/Cargo.toml` (members: `audio-core`, `engine-core`, `engine-dsp`, `backend-cpal`, `backend-null`, `backend-miniaudio`, `library*`, `codec`, `resampler`, `analyzer*`, `app-example`, `host-flutter`). Run cargo via `cargo --manifest-path crates/Cargo.toml …` (or `cd crates`). Desktop UI is `apps/gui-app` (Tauri + React; npm/moon globs `apps/*` / `packages/*`). Experimental Flutter host is `apps/gui-flutter` + `crates/host-flutter` via flutter_rust_bridge; Flutter/ninja pinned in `mise.toml`. rust-analyzer uses `.vscode/settings.json` `linkedProjects` for `crates/Cargo.toml` and `apps/gui-app/src-tauri/Cargo.toml`.
