...
/// Default Burn HTDemucs model id.
    ...
    match model_name {
        DEFAULT_MODEL => Ok(ModelManifest {
            ...
            artifacts: vec![Artifact {
                file: "htdemucs.safetensors".into(),
                // sha256 pinned from Hugging Face set-soft/audio_separation Demucs/htdemucs.safetensors
                sha256: "8193504cdfb3943adaf039b8acb524a46e87ebf232c383ac7a32c80a6578423e".into(),
                size_bytes: 84_030_696,
                url: "https://huggingface.co/set-soft/audio_separation/resolve/main/Demucs/htdemucs.safetensors".into(),
...
mod tests {
    ...
    #[test]
    fn builtin_manifest_htdemucs_burn_v1() {
        ...
        assert_eq!(art.file, "htdemucs.safetensors");
        assert_eq!(art.size_bytes, 84_030_696);
        assert!(art.url.contains("htdemucs.safetensors"));
    }
        ...
        let manifest = manifest_from_json(&format!(
            r#"{{
              ...
              "artifacts": [{{
                ...
                "sha256": "{sha}",
                "size_bytes": {},
                "url": "http://127.0.0.1:9/unreachable.safetensors"
              }}]
            }}"#,
            content.len()
        ));

        let expected = artifact_path(root.path(), &manifest, "m.safetensors", sha).expect("path");
    ...
    fn artifact_path_uses_models_root_only() {
        ...
        let manifest = manifest_from_json(
            r#"{
              "name": "htdemucs_burn_v1",
              "sample_rate": 44100
            }"#,
        );
        let path =
            artifact_path(root, &manifest, "htdemucs.safetensors", "abcdef0123456789").unwrap();
        assert_eq!(
            path,
            PathBuf::from("/tmp/mixar-models/htdemucs_burn_v1-abcdef01.safetensors")
        );
    }
    ...
    fn artifact_path_rejects_traversal_and_absolute_names() {
        ...
        assert!(artifact_path(root, &traversal, "m.safetensors", "abcdef01").is_err());
        ...
        assert!(artifact_path(root, &absolute, "m.safetensors", "abcdef01").is_err());
        ...
        assert!(artifact_path(root, &safe, "../../evil.safetensors", "abcdef01").is_err());
    }
